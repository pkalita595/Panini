...
X : E { $$ = $1; }
    ...
    | IF '(' E COND E ')' THEN E ';'  ELSE E ';' {
        ...
        printf("\t  cond = {| (unknown[%d]|unknown[%d]) (<|>|<=|>=|==|!=) (unknown[%d]|unknown[%d]) |} ;\n", $3, $5, $3, $5);

        printf(""
        "   unknown[%d] = (cond ? unknown[%d] : unknown[%d]);\n"
        "", p, $8, $11);
    
        for (int i=2; i<=last_id; i++){
            printf("\t  cond = {| (unknown[%d]|unknown[%d]) (<|>|<=|>=|==|!=) (unknown[%d]|unknown[%d]) |} ;\n", (i-1)*offset+$3,(i-1)*offset+$5, (i-1)*offset+$3, (i-1)*offset+$5);
            printf(""
            "   unknown[%d] = (cond ? unknown[%d] : unknown[%d]);\n"
            "", (i-1)*offset+p, (i-1)*offset+$8, (i-1)*offset+$11);
...
/*
    | IF '(' E COND E ')' THEN E ';' {
        printf("\t  cond = {| (unknown[%d]|unknown[%d]) (<|>|<=|>=|==|!=) (unknown[%d]|unknown[%d]) |} ;", $3, $5, $3, $5);

        printf(""
        "   unknown[%d] = (cond ? unknown[%d] : )\n"
        "", p, $8);
    
        for (int i=2; i<=last_id; i++){
            printf("\t  cond = {| (unknown[%d]|unknown[%d]) (<|>|<=|>=|==|!=) (unknown[%d]|unknown[%d]) |} ;", (i-1)*offset+$3,(i-1)*offset+$5, (i-1)*offset+$3, (i-1)*offset+$5);
            printf(""
            "   unknown[%d] = ( cond ? unknown[%d]: ) \n"
            "", (i-1)*offset+p, (i-1)*offset+$8);
